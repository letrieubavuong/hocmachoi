...
export const ChibiAvatar: React.FC<ChibiAvatarProps> = ({
  ...
  const sizeMap = {
    sm: { width: 64, height: 80, fontSize: 'text-xs' },
    md: { width: 96, height: 120, fontSize: 'text-sm' },
    lg: { width: 140, height: 175, fontSize: 'text-base' },
    xl: { width: 190, height: 230, fontSize: 'text-lg' },
  };
  ...
  return (
    <div className={`relative flex flex-col items-center select-none ${className}`}>
      ...
      {showName && name && (
        <div className="absolute -top-7 z-20 transition-all duration-300">
          <div className="px-3 py-1 bg-slate-900/90 text-white font-extrabold rounded-full border-2 border-purple-500 shadow-lg text-xs whitespace-nowrap flex items-center gap-1.5 backdrop-blur-md">
            {streak >= 2 && <span className="animate-pulse text-amber-400">🔥 {streak}</span>}
            <span>{name}</span>
            {shieldActive && <span className="text-blue-400 font-bold">🛡️</span>}
          </div>
        </div>
      )}
      ...
      {shieldActive && (
        <div 
          className="absolute inset-0 rounded-full border-4 border-cyan-400/80 bg-cyan-400/20 shadow-[0_0_25px_rgba(34,211,238,0.8)] animate-pulse z-10 pointer-events-none transform scale-110"
        />
      )}

      {/* Main SVG Chibi Body */}
      <div 
        className={`${isBouncing ? 'animate-bounce-slow' : ''} transition-transform duration-300 transform hover:scale-110`}
      ...
      >
        <svg viewBox="0 0 100 125" className="w-full h-full drop-shadow-xl">
          ...
          {customization.hatStyle === 'halo' && (
            <ellipse cx="50" cy="12" rx="22" ry="6" fill="none" stroke="#fbbf24" strokeWidth="4" className="animate-pulse" />
          )}
          ...
          <rect
            ...
            height="38"
            rx="12"
            fill={customization.outfitColor || '#6c5ce7'}
          />
          ...
          {/* Arms & Hands */}
          <circle cx="26" cy="80" r="7" fill={customization.skinColor} />
          <circle cx="74" cy="80" r="7" fill={customization.skinColor} />
          ...
          {/* Head */}
          <circle cx="50" cy="42" r="28" fill={customization.skinColor} />
          ...
          {customization.eyeType === 'star' && (
            <>
              <text x="33" y="44" fontSize="11" fill="#f59e0b">★</text>
              <text x="57" y="44" fontSize="11" fill="#f59e0b">★</text>
            </>
          )}
          ...
          {/* Mouth */}
          <path d="M 44 51 Q 50 56 56 51" stroke="#1e293b" strokeWidth="2.5" fill="none" strokeLinecap="round" />

          {/* Hairstyle */}
          {customization.hairStyle === 'short' && (
            <path d="M 22 36 Q 50 12 78 36 Q 74 24 50 20 Q 26 24 22 36 Z" fill={customization.hairColor} />
          )}
          {customization.hairStyle === 'spiky' && (
            <path d="M 22 38 L 30 18 L 40 26 L 50 14 L 60 26 L 70 18 L 78 38 Z" fill={customization.hairColor} />
          )}
          {customization.hairStyle === 'pigtails' && (
            <>
              <path d="M 22 36 Q 50 16 78 36 Q 50 20 22 36 Z" fill={customization.hairColor} />
              <circle cx="18" cy="36" r="10" fill={customization.hairColor} />
              <circle cx="82" cy="36" r="10" fill={customization.hairColor} />
            </>
          )}
          {customization.hairStyle === 'curly' && (
            <path d="M 22 40 Q 25 15 50 15 Q 75 15 78 40 C 85 25 65 10 50 10 C 35 10 15 25 22 40 Z" fill={customization.hairColor} />
          )}
          {customization.hairStyle === 'long' && (
            <>
              <rect x="20" y="32" width="10" height="36" rx="5" fill={customization.hairColor} />
              <rect x="70" y="32" width="10" height="36" rx="5" fill={customization.hairColor} />
              <path d="M 22 36 Q 50 16 78 36 Z" fill={customization.hairColor} />
            </>
          )}
          {customization.hairStyle === 'afro' && (
            <circle cx="50" cy="32" r="32" fill={customization.hairColor} opacity="0.9" />
          )}

          {/* Hats */}
          {customization.hatStyle === 'crown' && (
            <path d="M 32 20 L 41 8 L 50 16 L 59 8 L 68 20 Z" fill="#fbbf24" stroke="#d97706" strokeWidth="1.5" />
          )}
          ...
          {customization.hatStyle === 'wizardHat' && (
            <>
              ...
              <text x="46" y="20" fontSize="10" fill="#fbbf24">★</text>
            </>
          )}
          ...
          {customization.accessory === 'headphone' && (
            <>
              <path d="M 22 40 C 22 15 78 15 78 40" fill="none" stroke="#3b82f6" strokeWidth="4" />
          ...
          {customization.accessory === 'mask' && (
            <rect x="34" y="44" width="32" height="14" rx="4" fill="#ffffff" stroke="#cbd5e1" strokeWidth="1" />
          )}
        </svg>
      </div>
    </div>
